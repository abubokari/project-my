...
function Course() {
  ...
  useEffect(() => {
    let config = {
      ...
      url: `${baseUrl}/backend/course/allCourse`,
    };
  ...
  const handleDeleteDepartment = () => {
    if (deleteId) {
      let config = {
        method: "get",
        maxBodyLength: Infinity,
        url: `${baseUrl}/backend/course/delete/${deleteId}`,
      };
          ...
          {rows.length !== 0 ? (
            rows.map((el, index) => (
              <Table.Row key={index}>
                ...
                <Table.Cell>
                  {el.departmentId.title}
                </Table.Cell>
                <Table.Cell>
                  <Button.Group>
                    <Button color="blue">
                      <NavLink to={`/course/edit/${el._id}`}>Edit</NavLink>
                    </Button>
                    <Button.Or />
                    <Button
                      color="google plus"
